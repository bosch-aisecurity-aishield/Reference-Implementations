...
Date: 9/14/2022
Author: Amlan Jyoti
Description: Basic script used to deploy a Scikit-learn model to AWS Sagemaker.
"""
...
def model_fn(model_dir):
    model = pickle.load(open(os.path.join(model_dir, "defense_model_banking_svc_scaled.pkl"),"rb"))
    return model
...
def predict_fn(input_data, model):
    attack_proba = model.predict_proba(input_data)[0][1]
    return attack_proba
